...
In the first iteration of the project, I used pandas-datareader, an extremely convenient library which can load stock data straight into pandas. However, after Yahoo Finance changed their UI, datareader no longer worked, so I switched to Quandl, which has free stock price data for a few tickers, and a python API. 
...
However, referring to the example of AAPL above, if our snapshot includes fundamental data for 28/1/17 and we want to see the change in price a year later, we will get the nasty surprise that 28/1/2018 is a Saturday. Does this mean that we have to discard this snapshot?

By no means – data is too valuable to discard. As a workaround, I instead decided to 'fill forward' the missing data, i.e assume that the stock price on Saturday 28/1/2018 is equal to the stock price on Friday 27/1/2018.

# Features
Below is a list of some of the interesting variables that are available on Yahoo Finance.

<h1>Valuation Measures</h1>
<ul>
<li>'Market Cap'</li>
...
<code>python parsing_keystats.py</code>
...
<code>python backtesting.py</code>
...
<code>python current_data.py</code>

The script will then begin downloading the HTML into the forward/ folder within your working directory, before parsing this data and outputting the file <code>forward_sample.csv</code>. You might see a few miscellaneous errors for certain tickers (e.g 'Exceeded 30 redirects.'), but this is to be expected.
...
<code>python stock_prediction.py</code>

You should get something like this:

<code>21 stocks predicted to outperform the S&P500 by more than 10%:
NOC FL SWK NFX LH NSC SCHL KSU DDS GWW AIZ ORLY R SFLY SHW GME DLX DIS AMP BBBY APD</code>
...
I therefore recommend that you run the tests after you have run all the other scripts (except, perhaps, stock_prediction.py).

To run the tests, simply enter the following into a terminal instance in the project directory:

<code>pytest -v</code>

Please note that it is not considered best practice to include an <code>__init__.py</code> file in the <code>tests/</code> directory (see here for more), but I have done it anyway because it is uncomplicated and functional.
...
<ul>
<li>Explore the other subfolders in Sentdex's intraQuarter.zip</li>
<li>Parse the annual reports that all companies submit to the SEC (have a look at the Edgar Database)</li>
...
<li>In this project, I have just ignored any rows with missing data, but this reduces the size of the dataset considerably. Are there any ways you can fill in some of this data?</li>
...
<li>Some form of feature engineering</li>
  <li>e.g, calculate Graham's number and use it as a feature</li>
  <li>some of the features are probably redundant. Why not remove them to speed up training?</li>
<li>Speed up the construction of keystats.csv</li>
